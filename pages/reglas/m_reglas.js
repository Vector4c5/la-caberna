import Header from "@/components/common/Header";
import { useEffect, useState } from "react";
import { useRouter } from "next/router";

export default function Reglas() {
        const router = useRouter();
        const [classes, setClasses] = useState([]);
    
        useEffect(() => {
            const fetchClasses = async () => {
                try {
                    const response = await fetch("https://www.dnd5eapi.co/api/rule-sections");
                    const data = await response.json();
                    const formattedClasses = data.results.map((classItem, index) => ({
                        id: index + 1,
                        title: classItem.name,
                        description: `Has sufrir a tus jugadores con ${classItem.name}`,
                        link: `/reglas/${classItem.index}`,
                    }));
                    setClasses(formattedClasses);
                } catch (error) {
                    console.error("Error:", error);
                }
            };
    
            fetchClasses();
        }, []);
    
        const handleClaseClick = (index) => {
            router.push(`/reglas/${index}`);
        };


    return (
        <div className="flex flex-col items-center w-full h-full bg-black p-10">
            <img
                src="/Fondo_Biblioteca.jpeg"
                alt="Fondo bienvenida"
                className="fixed top-0 left-0 w-full h-full object-cover opacity-15 z-0"/>
                <div className="w-full h-auto flex justify-center mb-5">
                <Header/>
                </div>
                <div
                className="container flex flex-col  items-center gap-3 w-10/12 h-auto border-solid border-white border-b-2 border-t-2 p-5 
                animate-fade-in-down"
                style={{
                    boxShadow:
                        "0 10px 15px -3px rgba(255, 255, 255, 0.1), 0 -10px 15px -3px rgba(255, 255, 255, 0.1)",
                    fontFamily: "'Press Start 2P', cursive",
                }}
            >
                <h1 className="text-center text-4xl w-full text-white">Reglas de roll D&D</h1>
                <h2 className="text-xl text-center text-cyan-400">Expande tu conocimieto</h2>
            </div>
            <div className="flex flex-col items-center mt-12 w-10/12 border-4 border-yellow-800 rounded-lg">
                {classes.map((classItem) => (
                    <div key={classItem.id} className="flex justify-center w-11/12 ">
                        <button
                            onClick={() => handleClaseClick(classItem.link.split("/").pop())}
                            className="container group w-full h-32 px5 py2.5 relative font-medium
                            text-white inline-block border-b-2 border-yellow-500"
                        >
                            <div className="absolute inset-0 flex flex-col justify-center items-start px-10">
                                <h3
                                    className="text-2xl group-hover:text-3xl transition-all duration-300"
                                    style={{
                                        fontFamily: "'Press Start 2P', cursive",
                                    }}
                                >
                                    {classItem.title}
                                </h3>
                            </div>
                           {/* <div className="absolute inset-0 flex flex-col justify-center items-center">
                                <span className="w-full h-full absolute opacity-90"></span>
                                <p
                                    className="relative bg-white bg-opacity-80 rounded-lg m-6 p-4 
                                    text-center text-sm text-black font-['Press_Start_2P']"
                                >
                                    {classItem.description}
                                </p>
                            </div>  */}
                        </button>
                    </div>
                ))}
            </div>
        </div>
    );
}