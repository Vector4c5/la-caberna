...
export default function Reglas() {
  ...
  return (
    <div className="flex flex-col items-center w-full h-full bg-black p-10">
      <img
      ...
      >
        ...
        <h2 className="text-xl text-center text-cyan-400">
          Expande tu conocimieto
        </h2>
      </div>
      <div className="flex flex-col items-center mt-12 w-10/12 z-20">
        {classes.map((classItem) => (
          <div key={classItem.id} className="flex justify-center w-11/12 ">
            <button
              onClick={() => handleClaseClick(classItem.link.split("/").pop())}
              className="container group w-full h-32 px5 py2.5 relative font-medium
            ...
            >
              <div className="absolute inset-0 flex flex-col justify-center items-start px-10">
                <h3
                  className="text-2xl  group-hover:translate-x-5 group-hover:text-gray-400 transition-all duration-300"
                  style={{
                    fontFamily: "'Press Start 2P', cursive",
                  }}
                >
              ...
              <div className="opacity-0 group-hover:opacity-100 transition-sll ease-out duration-300">
              <svg
